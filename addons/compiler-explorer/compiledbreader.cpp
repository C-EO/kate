...
QString CompileDBReader::argsForFile(const QString &compile_commandsPath, const QString &file)
{
    ...
    if (!f.open(QFile::ReadOnly)) {
        ...
        qDebug() << "Failed to load compile_commands: " << f.errorString();
        return {};
    }
    ...
    if (error.error != QJsonParseError::NoError) {
        qDebug() << "Failed to read compile_commands: " << error.errorString();
        return {};
    }

    if (!cmdCmds.isArray()) {
        qDebug() << "Invalid compile_commands, root element is not an array";
        return {};
    }
